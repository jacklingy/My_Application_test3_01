...
public class MainActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        initMovie();
        movieAdapter = new MovieAdapter(MainActivity.this, R.layout.movie_item, movieList);
    ...
    public boolean onOptionsItemSelected(@NonNull MenuItem item) {
        switch (item.getItemId()) {
            case R.id.menuAdd:
                ...
                movieList.add(m, newMovie);
                //动态
                movieAdapter.notifyDataSetChanged();
                //提示
                Toast.makeText(getApplicationContext(), "已随机添加一部电影!\n" +
                        newMovie.getName(), Toast.LENGTH_SHORT).show();
                break;
            case R.id.menuDecline:
                ...
                if (movieList.size() > 1) {
                    movieList.remove(movieList.get(m2));
                    movieAdapter.notifyDataSetChanged();
                    Toast.makeText(getApplicationContext(), "已随机删除一部电影！", Toast.LENGTH_SHORT).show();
